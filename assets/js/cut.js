...
(function () {
  ...
  /* ---- يُستخدم في (الربط العكسي: قطعة ← طبخة) ---- */
  const dishes = T.dishesForCut(cut.slug);
  if (dishes.length) {
    const DICON = {
    ...
    $("cutDishes").innerHTML = dishes.map(d => `<a class="dish" href="dish.html?d=${d.slug}">
      ...
      <b>${d.name}</b><span>${d.perPerson}</span></a>`).join("");
  }

  /* ---- structured data (schema.org) — يعكس ما هو معروض فقط ---- */
  (function ld() {
    const base = location.href.split("?")[0].replace(/cut\.html$/, "");
    const data = [{
      "@context": "https://schema.org", "@type": "Product",
      name: cut.name, alternateName: cut.en, sku: cut.code,
      description: cut.origin + " " + cut.why,
      category: cut.animal + " / " + primal.name,
      offers: {
        "@type": "Offer", price: String(cut.price), priceCurrency: "SAR",
      ...
      itemListElement: [
        ...
        { "@type": "ListItem", position: 2, name: "دليل القطعيات", item: base + "cuts.html" },
        { "@type": "ListItem", position: 3, name: cut.name }
      ]
    }];
